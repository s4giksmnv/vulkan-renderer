...
void Engine::run() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (elapsed >= 1.0) {
            ...
            title << "Vulkan Window, FPS: " << frames/elapsed;
...
void Engine::createGraphicsPipeline() {
    ...
    vertStageInfo.pSpecializationInfo = nullptr; // allows us to specify shader constants
...
void Engine::createVertexBuffer() {
    ...
    memcpy(data, vertices.data(), sizeof(vertices[0])*vertices.size()); // copy vertices into data, which is bound to stagingBufferMemory
    vkUnmapMemory(device, stagingBufferMemory);
